...
経済産業省が公表する4つの統計を収録しています。

- [経済産業省 経済解析室](https://www.meti.go.jp/statistics/tyo/sanzi/)の第３次産業活動指数（2020年基準）。
  サービス産業の生産活動を業種別・月次の指数（2020年=100）で示します。
- [経済産業省](https://www.meti.go.jp/statistics/tyo/syoudou/)の商業動態統計調査。
  卸売業・小売業の商業販売額を業種別・月次で、百貨店・スーパー・コンビニエンスストア・
  家電大型専門店・ドラッグストア・ホームセンターの販売額を商品分類別・月次で示します。
...
- [経済産業省 資源エネルギー庁](https://www.enecho.meti.go.jp/statistics/electric_power/ep002/)の電力調査統計。
  ...
  自家用発電所の発電所数・最大出力は地域別・半期（9月末／3月末）で、
  自家用発電の発電電力量・送電電力量・自家消費電力量は地域別・原動力別・月次で示します。
  2024年度以降の自家用発電は都道府県別・原動力別で、発電電力量と最大出力は月次、
  発電所数は年度で示します。
...
## テーブル: commercial_sales_by_industry

業種×月を1レコードとする商業販売額です。1980年1月以降を収録します。
指数ではなく販売額そのもので、単位は10億円です。全国の値のみで、地域別の内訳はありません。

- fiscal_year: 年度（INTEGER、4月始まり）
- year: 年（INTEGER）
- month: 月（INTEGER）
- year_month: 年月（VARCHAR、YYYYMM）
- industry_order: 原典の統計表での業種の並び順（INTEGER）
- industry_name: 業種名（VARCHAR）
- industry_name_en: 業種名（VARCHAR、英語）
- sales_value_billion_yen: 商業販売額（DOUBLE、10億円）

### 利用上の注意

商業計・卸売業計・小売業計と、その内訳の業種、内訳を足し上げた参考系列（「参考１(自動車小売業、
機械器具小売業、燃料小売業、医薬品化粧品小売業、その他小売業の計)」など）が同じ列に並びます。
業種を絞らずに合計すると二重計上になります。industry_order は原典の統計表での並び順で、
合計とその内訳の対応を追うのに使えます。

小売業の内訳には無店舗小売業（通信販売・ネット販売）が別掲されています。

販売額は原典の公表値そのままで、調査の見直しをまたぐ接続処理はしていません。次の時点で
水準に不連続があります。

| 時点 | 理由 |
| --- | --- |
| 1998年4月 | 平成11年商業統計調査で既設の対象事業所を捕そく |
| 1999年4月 | 自動車卸売業における販売額の定義変更 |
| 2015年7月 | 無店舗小売業の表章を開始 |
| 2020年3月 | 水準の調整 |
| 2025年1月 | 水準の調整 |

1989年以前も、貴金属ディーリングに係る取引額の除外（1990年まで遡及）と日本標準産業分類の
改訂（1989年まで遡及）で不連続です。原典は、これらの期間をまたぐ前年比をリンク係数で算出した
と注記しており、係数そのものは原典の注シートにあります。このテーブルには適用していません。

集計に必要なデータが無い月（原典の「***」）と秘匿された値（「X」）は行ごと落としています。

## テーブル: retail_sales_by_business_type_and_item

業態×商品分類×月を1レコードとする業態別・商品別販売額です。販売額の単位は百万円、
店舗数は店です。全国の値のみで、地域別の内訳はありません。

- fiscal_year: 年度（INTEGER、4月始まり）
- year: 年（INTEGER）
- month: 月（INTEGER）
- year_month: 年月（VARCHAR、YYYYMM）
- business_type: 業態（VARCHAR）
- item_order: 原典の統計表での商品分類の並び順（INTEGER）
- item_name: 商品分類名（VARCHAR）
- item_name_en: 商品分類名（VARCHAR、英語）
- sales_value_million_yen: 販売額（DOUBLE、百万円）
- establishments: 期末の店舗数（BIGINT、店）

業態と収録期間は次のとおりです。

| business_type | 収録期間 | 項目数（合計・店舗数を含む） |
| --- | --- | --- |
| 百貨店・スーパー | 1980年1月〜 | 13 |
| 百貨店 | 1980年1月〜 | 13 |
| スーパー | 1980年1月〜 | 13 |
| コンビニエンスストア | 1997年4月〜 | 7 |
| 家電大型専門店 | 2014年1月〜 | 18 |
| ドラッグストア | 2014年1月〜 | 11 |
| ホームセンター | 2014年1月〜 | 11 |
...
「百貨店・スーパー」は百貨店とスーパーの合計です。業態を絞らずに合計すると二重計上になります。
商品分類も合計（「合計」「商品販売額」）と内訳が同じ列に並ぶため同様です。

item_name は業態ごとに体系が違い、同じ名前の商品分類が1つの業態に複数あります（家電大型専門店の
「その他」は item_order 15 と 17 の2つ）。商品分類を指すときは business_type と item_order の
組で指します。

sales_value_million_yen と establishments はどちらか一方だけが入ります（店舗数の行では
販売額が NULL）。店舗数を持つのはコンビニエンスストア・家電大型専門店・ドラッグストア・
ホームセンターだけです。

販売額は原典の公表値そのままで、調査の見直しをまたぐ接続処理はしていません。次の時点で
水準に不連続があります。

| business_type | 不連続の時点 |
| --- | --- |
| 百貨店・スーパー / 百貨店 / スーパー | 1991年7月・1994年7月・2010年7月・2013年7月・2015年7月・2020年3月 |
| コンビニエンスストア | 2012年1月・2017年1月・2021年3月 |
| 家電大型専門店 | 2018年1月・2020年3月 |
| ドラッグストア | 2015年1月・2017年1月・2018年1月・2019年1月 |
| ホームセンター | 原典に不連続の注記なし |

原典は、不連続の前の期間にリンク係数を乗じると後の期間と接続すると注記しており、係数そのものは
原典の注シートにあります。このテーブルには適用していません。直近の年の1月分以降は年間補正で
値が変わることがあります。

集計に必要なデータが無い月（原典の「***」）と秘匿された値（「X」）は行ごと落としています。
コンビニエンスストアは1997年1月から表に行がありますが、値が入るのは1997年4月分からです。
...
- 2024年度以降は原典が「自家用発電所等運転半期報集計結果」（発電電力量や送電電力量を含む
  ...
  2024年度以降の発電所数は captive_power_plants_by_prefecture が年度で、最大出力は
  captive_power_generation_by_prefecture が月次で持ちます。

## テーブル: captive_power_generation_by_area

電力調査統計の統計表 5-(2)「自家用発電実績」を、地域×月×原動力で1レコードとした月次データです。
2017年4月から2024年3月までの84か月・19,440行を収録します。10地域のみで、全国合計の列はありません。
...
- year_month: 年月（VARCHAR、YYYYMM）
- area_name: 地域名（VARCHAR、北海道・東北・関東・中部・北陸・近畿・中国・四国・九州・沖縄）
- power_source_group: 原動力の大分類（VARCHAR、水力／火力／火力（燃料別）／原子力／新エネルギー等／その他／合計）
- power_source_name: 原動力の小分類（VARCHAR、大分類が小分類を持たないときは大分類と同じ値）
- is_reference: 再掲の行か（BOOLEAN）
- generation_mwh: 発電電力量（DOUBLE、MWh）
- station_use_and_loss_mwh: 所内及び損失電力量（DOUBLE、MWh）
- transmission_to_utilities_mwh: 電気事業者への送電電力量（DOUBLE、MWh）
- transmission_to_specified_supply_mwh: 特定供給の相手方への送電電力量（DOUBLE、MWh）
- transmission_to_other_operators_mwh: その他事業者への送電電力量（DOUBLE、MWh）
- transmission_total_mwh: 電気事業者等への送電電力量の合計（DOUBLE、MWh）
- transmission_via_exchange_mwh: 送電電力量の合計のうち卸電力取引所を通じた取引（DOUBLE、MWh、内数）
- self_consumption_mwh: 自家消費電力量（DOUBLE、MWh）

power_source_group と power_source_name の組み合わせは次の23通りです（蓄電池が足された2023年度以降は24通り）。

| power_source_group | power_source_name |
| --- | --- |
| 水力 | 水力 |
| 火力 | 汽力 / ガスタービン / 内燃力 / 計 / コージェネレーション |
| 火力（燃料別） | 石炭 / ＬＮＧ / 石油 / ＬＰＧ / その他ガス / 瀝青質混合物 / その他 / 計 |
| 原子力 | 原子力 |
| 新エネルギー等 | 風力 / 太陽光 / 地熱 / 蓄電池 / 計 / バイオマス / 廃棄物 |
| その他 | 燃料電池等 |
| 合計 | 合計 |

### 利用上の注意

- 対象は自家用発電所（自らの需要のために発電する設備）です。power_generation_by_prefecture と
  power_generation_by_operator は電気事業者の発電実績なので対象が重ならず、足し合わせても
  国内の発電量の全量にはなりません。
- 発電所数・最大出力を持つ captive_power_plants_by_area と対象は同じで、地域名も揃えてあります。
  ただしあちらは9月末・3月末の年2回なので、突き合わせられるのは3月と9月だけです。
- 合計・内訳・再掲が同じ列に並びます。is_reference が true の行（火力のコージェネレーション、
  火力（燃料別）のすべて、新エネルギー等のバイオマスと廃棄物）と power_source_name が「計」の
  行を混ぜて足すと二重計上になります。
- 火力（燃料別）は火力の原動力別（汽力・ガスタービン・内燃力）を燃料別に置き換えた再掲です
  （原典の備考3）。複数の燃料を使う場合は主要な燃料に計上されます（同 備考4）。
- 新エネルギー等のバイオマスと廃棄物は火力の原動力別からの再掲です（原典の備考5）。
  火力の計と合計には含まれ、新エネルギー等の計には含まれません。
- generation_mwh は station_use_and_loss_mwh・transmission_total_mwh・self_consumption_mwh に
  分かれます。transmission_total_mwh は送電先3つ（電気事業者・特定供給の相手方・その他事業者）の
  和で、transmission_via_exchange_mwh はその内数です。
- 再掲の行は発電電力量以外の欄が原典で「－」になっており、NULL として収録しています。
- 地域は経済産業局の管轄です（北陸は北陸支局、沖縄は沖縄総合事務局）。都道府県別の内訳は
  原典に無いため、都道府県別の表とは結合できません。
- 個々の数値の合計と合計欄は四捨五入の関係で一致しない場合があります（原典の備考1）。
  generation_mwh と (station_use_and_loss_mwh + transmission_total_mwh + self_consumption_mwh) の
  差が1 MWh を超える行が2022年度までに20行あります（最大4 MWh）。
- 蓄電池は2023年度に原典へ足された行で、それ以前の月にはありません。発電電力量の欄は原典で
  空欄（120セル中102）か0で、所内及び損失電力量・送電電力量・自家消費電力量だけが入ります。
  そのため2023年度は新エネルギー等の計と合計の行で上の等式が成り立たないことがあります
  （46行・最大291 MWh）。
- 2018年度の九州は瀝青質混合物・原子力・燃料電池等の発電電力量が原典で空欄です（各12か月）。
  他の年度・地域は0が入っています。
- 2017年度下期のシートは原典の側で全国合計の列が10地域の和と合いません（217セル）。
  このシートには「H31.1.28一部修正」の注記があり、地域の値だけが直され全国合計の列が
  直っていないためです。例えば2017年10月の火力・汽力の発電電力量は全国合計の欄が
  4,765,379 MWh、10地域の和が 4,780,109 MWh です。年度計のシートは10地域の和と整合するので、
  収録している地域の値の側が新しいものです。
- 同じ一部修正の取り残しが北陸の新エネルギー等の計にもあります。2017年10月から2018年3月までの
  6か月で、計が内訳（風力＋太陽光＋地熱）の和より 862〜11,556 MWh 小さくなっています。
  合計の行は内訳の側と整合するので、新エネルギー等を見るときは内訳から足し直してください。
- 2021年度上期の近畿は、4月から9月までの全ての値が2020年10月から2021年3月までと1行残らず
  同じです。原典の写し違いとみられます。近畿を時系列で見るときはこの6か月を除いてください。
- 火力（燃料別）の計は火力の原動力別の計と一致しないことがあります（840組中36組）。
  燃料別の計と燃料別の内訳の和が合わない組も9つあり、うち四国の2021年12月・2022年2月・
  2022年3月は計が0で内訳だけに値が入っています。
- 2016年度は原典が別のレイアウトで、地域×月の1つの値しか持ちません。しかもそれは
  発電電力量ではなく自家消費（業務用を除く）と特定供給の合計（原典の注1）で、系列が
  つながらないため取り込んでいません。
- 2016年4月に施行された改正電気事業法（第2弾改正）により、発電事業に該当することになった
  発電所・発電設備は報告の対象外です（原典の注2）。
- 公表時点を示す欄が原典に無いため、published_as_of は持っていません。
- 2024年度以降は原典が「自家用発電所等運転半期報集計結果」（発電電力量や送電電力量を含む
  別レイアウトの1ファイル）に統合され、この統計表の形式では配布されていません。
  2024年度以降は captive_power_generation_by_prefecture に続きます。

## テーブル: captive_power_generation_by_prefecture

電力調査統計の統計表 5「自家用発電所等実績」を、都道府県×月×原動力で1レコードとした
月次データです。2024年4月から2026年3月までの24か月・28,200行を収録します。
47都道府県のみで、全国合計のブロックはありません。

- fiscal_year: 年度（INTEGER、4月始まり）
- year: 年（INTEGER、暦年）
- month: 月（INTEGER）
- year_month: 年月（VARCHAR、YYYYMM）
- pref_code: 都道府県コード（VARCHAR、全国地方公共団体コードの上2桁）
- pref_name: 都道府県名（VARCHAR）
- power_source_group: 原動力の大分類（VARCHAR、水力／火力／火力（燃料別）／原子力／新エネルギー／燃料電池等／蓄電池／合計）
- power_source_name: 原動力の小分類（VARCHAR、大分類が小分類を持たないときは大分類と同じ値）
- is_reference: 再掲の行か（BOOLEAN）
- capacity_kw: 最大出力（DOUBLE、kW。当該月の値）
- generation_mwh: 発電電力量（DOUBLE、MWh）
- station_use_and_loss_mwh: 所内及び損失電力量（DOUBLE、MWh）
- transmission_to_utilities_mwh: 電気事業者への送電電力量（DOUBLE、MWh）
- transmission_to_specified_supply_mwh: 特定供給の相手方への送電電力量（DOUBLE、MWh）
...
power_source_group と power_source_name の組み合わせは次の25通りです。

| power_source_group | power_source_name |
| --- | --- |
| 水力 | 水力 |
| 火力 | 計 / 汽力 / ガスタービン / 内燃力 / コージェネレーション |
| 火力（燃料別） | 計 / 石炭 / 石油 / 液化石油ガス / 天然ガス / その他ガス / 瀝青質混合物 / バイオマス / 廃棄物 / その他 |
| 原子力 | 原子力 |
| 新エネルギー | 計 / 風力 / 太陽光 / 地熱 / バイオマス |
| 燃料電池等 | 燃料電池等 |
| 蓄電池 | 蓄電池 |
| 合計 | 合計 |
...
- captive_power_generation_by_area（2017年度〜2023年度）の後継です。原典が2024年度から
  統計表 5-(1)「自家用発電所数、出力」と 5-(2)「自家用発電実績」を1つに統合し、同時に内訳が
  経済産業局の10地域から47都道府県へ細かくなりました。列の意味と単位は揃えてありますが、
  区分の作りが変わっているので系列として繋ぐ前に定義を確かめてください。
- 電力量の単位は MWh です。原典の単位は kWh（原典の注8）なので、原典の値と比べるときは
  1,000倍の差に注意してください。最大出力は kW です。
- capacity_kw は当該月の最大出力です。原典の年度計の欄は期末（3月）の値なので（原典の注7）、
  年度の代表値が要るときは3月の行を使ってください。
- 発電所数はこの表にありません。原典が年度計の欄にしか持たず、月次の欄は最大出力と電力量だけ
  なので、captive_power_plants_by_prefecture が年度の粒度で持っています。
- 対象は自家用発電所（自らの需要のために発電する設備）です。power_generation_by_prefecture と
  power_plants_by_prefecture は電気事業者の発電実績・発電所なので対象が重ならず、足し合わせても
  都道府県の発電量・発電設備の全量にはなりません。
- 合計・内訳・再掲が同じ列に並びます。is_reference が true の行（火力のコージェネレーション、
  火力（燃料別）のすべて、新エネルギーのバイオマス）と power_source_name が「計」の行を
  混ぜて足すと二重計上になります。
- 火力（燃料別）は火力の原動力別（汽力・ガスタービン・内燃力）を燃料別に置き換えた再掲です
  （原典の注3）。複数の燃料を使う場合は主要な燃料に計上されます（同 注4）。
- 新エネルギーのバイオマスは火力（燃料別）のバイオマスからの再掲です（原典の注5）。
  captive_power_generation_by_area とは扱いが違い、こちらは新エネルギーの計に含まれます。
  そのため合計は 水力＋火力の計＋原子力＋新エネルギーの計＋燃料電池等＋蓄電池 から
  バイオマスの再掲を差し引いた値です。廃棄物の行は新エネルギーに無く、火力（燃料別）だけが持ちます。
- 蓄電池の generation_mwh は充電電力量です（原典の注6）。発電した電力量ではありません。
- generation_mwh は station_use_and_loss_mwh・transmission_total_mwh・self_consumption_mwh に
  分かれます。transmission_total_mwh は送電先3つ（電気事業者・特定供給の相手方・その他事業者）の
  和で、transmission_via_exchange_mwh はその内数です。
- 個々の数値の合計と合計欄は四捨五入の関係で一致しない場合があります（原典の注1）。
- 原典の食い違いをそのまま収録しています。
  - 2024年度の岡山県は火力（燃料別）の計が原動力別の計より小さく、12か月すべてでずれます
    （年度計の発電電力量で467,530 MWh、最大出力で145,000 kW）。
  - 2024年度の茨城県は太陽光の発電電力量が所内損失・送電・自家消費の和より小さく、12か月すべてで
    ずれます（年度計で15.6 MWh）。新エネルギーの計と合計の行にも同じ差が出ます。
  - 2025年度の北海道の蓄電池は、内数であるはずの transmission_via_exchange_mwh が
    transmission_total_mwh を上回る月が7つあります（2025年4月〜10月）。
  - self_consumption_mwh が負の行が165、station_use_and_loss_mwh が負の行が5あります。
- 原典の全国合計のブロックは収録していません（47都道府県の和と一致することを確認しています）。
  全国の数字が要るときは都道府県を合算してください。

## テーブル: captive_power_plants_by_prefecture

電力調査統計の統計表 5「自家用発電所等実績」のうち発電所数を、都道府県×年度×原動力で
1レコードとした年度データです。2024年度と2025年度の2年度・2,350行を収録します。
47都道府県のみで、全国合計のブロックはありません。
...
- power_source_name: 原動力の小分類（VARCHAR、大分類が小分類を持たないときは大分類と同じ値）
- is_reference: 再掲の行か（BOOLEAN）
- plant_count: 自家用発電所数（INTEGER）

power_source_group と power_source_name の組み合わせは captive_power_generation_by_prefecture と
同じ25通りです。

### 利用上の注意

- 粒度が年度なのは、原典が発電所数を年度計の欄にしか持たないためです。同じ統計表の
  最大出力と発電電力量は月次で、captive_power_generation_by_prefecture にあります。
  年度と月で粒度が違うので、両表の行を1対1に対応づけることはできません。
- 原典は年度計の最大出力についてのみ期末（3月）の値と注記しており（原典の注7）、
  発電所数がどの時点の数かは書いていません。
- 2023年度末（2024年3月末）で切れている captive_power_plants_by_area の続きにあたる系列です。
  ただしあちらは10地域で原動力を列に持ち、新エネルギー等の計にバイオマスの再掲を含まないため、
  繋ぐには区分を揃え直す必要があります。
- 対象は自家用発電所（自らの需要のために発電する設備）です。power_plants_by_prefecture と
  power_plants_by_operator は電気事業者の発電所を数えたもので対象が重ならず、足し合わせても
  都道府県の発電設備の全量にはなりません。
- 報告の対象は発電設備の合計出力が1,000kW以上の自家用発電所です。複数の発電方式を持つ
  発電所は方式ごとに1つとして数えます。
- 合計・内訳・再掲が同じ列に並びます。is_reference が true の行（火力のコージェネレーション、
  火力（燃料別）のすべて、新エネルギーのバイオマス）と power_source_name が「計」の行を
  混ぜて足すと二重計上になります。
- 合計は 水力＋火力の計＋原子力＋新エネルギーの計＋燃料電池等＋蓄電池 からバイオマスの
  再掲を差し引いた値です（新エネルギーの計がバイオマスの再掲を含むため）。
  火力の計は 汽力＋ガスタービン＋内燃力、新エネルギーの計は 風力＋太陽光＋地熱＋バイオマス です。
- 原子力の行は全都道府県・全年度で0です。
- 原典の食い違いをそのまま収録しています。2024年度の岡山県は火力（燃料別）の計が51、
  火力の原動力別の計が52で、1か所ずれます（同じ県・同じ年度は発電電力量でもずれます）。
- 原典の全国合計のブロックは収録していません（47都道府県の和と一致することを確認しています）。
  全国の数字が要るときは都道府県を合算してください。
...
main.py が4つの統計の公開 Excel を取得して CSV へ整形し、dbt build で各テーブルを再生成する。
...
黙って飛ばさずに失敗させている。自家用発電実績だけは月次シートではなく上期・下期の2シートに
月ごとの塊が縦に積まれる形なので、塊の切れ目を大分類の見出しで拾い、6か月が揃わなければ
失敗させている。2024年度以降の自家用発電所等実績は年度ごとに1シートで、都道府県のブロックが
縦に積まれ年度計と12か月が横に並ぶ。原典が持つ全国合計のブロックと47都道府県の和が
一致することを取得のたびに確かめ、行がずれて読めていれば失敗させている。
発電所数を持つのは年度計の欄だけなので、1度の読み取りで月次の実績と年度の発電所数を
別々の CSV へ書き分けている。年度計の最大出力は期末（3月）の値で月次と重なるため
持たず、重なっていることを取得のたびに確かめている。
電力量の単位はこの統計表だけ kWh（他は 1,000kWh）なので、単位の注記が変わったら
失敗させている。事業者別の電力需要実績は
...
商業動態統計調査は時系列データの統計表を取得する。統計表ごとに列の構成も見出しの段数も違うが、
「時間軸コード」「年月」を持つ行が見出し行で、その1行上に日本語の項目名、さらに上の行に単位が
入る作りは共通しているため、見出し行の位置から単位の入った列だけを値として拾っている。
年・年度・四半期の集計表と前年比の表は取らない（月次の実額があれば SQL で作れる）。
年月は時間軸コードと年月の表記を突き合わせ、食い違えば失敗させている。

資源エネルギー庁と経済産業省のサイトは CloudFront + AWS WAF の challenge action で保護されており、
短時間に続けて取得すると HTTP 202 と検証ページが返る。取得側は間隔を空けて取り直す。
...
- 「商業動態統計調査」（経済産業省）（https://www.meti.go.jp/statistics/tyo/syoudou/）を加工して作成。
