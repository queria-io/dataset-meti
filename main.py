...
def main() -> None:
    ...
    logger.info("6/10: power_generation_operator (電力調査統計 発電実績 事業者別)")
    ...
    logger.info("7/10: power_municipality (電力調査統計 市町村別需要電力量・逆潮流量)")
    ...
    logger.info("8/10: power_plants (電力調査統計 都道府県別発電所数、出力)")
    rows = power_plants.download_and_parse(PLANTS_CSV_PATH)
    logger.info(f"  meti_power_plants.csv: {rows} rows")

    logger.info("9/10: power_thermal_fuel (電力調査統計 火力発電燃料実績)")
